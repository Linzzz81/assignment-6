...
object MonadOps {
  ...
  // 6 points.
  def mapFuture[X](xfs: Seq[Future[X]])(implicit executor: ExecutionContext): Seq[Future[Either[Throwable, X]]] = for (xfi <- xfs) yield sequence(xfi) // TO BE IMPLEMENTED
  ...
  // 7 points.
  def sequence[X](xi: Either[Throwable, X]): Option[X] = xi match {
    case Left(x) => None
    case Right(x) => Some(x)
  } // TO BE IMPLEMENTED
