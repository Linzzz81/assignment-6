...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(n: Node): Seq[Try[URL]] = for(ni <- n \\ "a"; href = ni\@"href") yield createURL(Some(u), href) // TO BE IMPLEMENTED
    ...
    // 9 points.
    for(gu <- getURLContent(u); mgl <- MonadOps.asFuture(getLinks(gu))) yield mgl // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    Future.sequence(for(us2i <- us2) yield MonadOps.sequence(wget(us2i)))
  }
